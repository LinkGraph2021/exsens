...
var header = {
  init: function(){
    ...
    if ($(window).width() > 798) {
      ...
      $defaultHeader.find('a').attr('tabindex', 1);
...
var video = {
  init: function(){
    ...
    this.videoElementObjects = [];
    var $videoElements = $('[data-video-element]');
    $.each($videoElements, function(index, element) {
      var $element = $(element);
      var sectionID = $element.parents('.shopify-section').attr('id');
      video.setup($element, sectionID);
    })

    // Set up plyr for newly embedded video
    var featuredVideos = document.querySelectorAll('[data-html5-video], [data-external-video]');
    ...
    $.each(this.videoElementObjects, function(index, video) {
      $.each(featuredVideoPlayers, function(index, player) {
        var $videoWrapper = $(video.element).parents('.video-wrapper');
        var videoID;
        var playerID;
        ...
        if (playerID == videoID) {
          ...
          // Video will be hidden using CSS if autoplay disabled
          ...
          if (video.isAutoplay) {
            ...
            if ($(window).width() > 768 || player.isYouTube || player.isVimeo) {
              ...
              // Remove '.video-is-hidden' class from video wrapper
              $(video.element).parents('.video-wrapper').removeClass('video-is-hidden');

              // Show video
              $(video.element).show();

              // If display text over video unchecked
              if ($(video.videoTextContainer).hasClass('display-text-over-video--false')) {
                $(video.videoTextContainer).hide();
              } else {
                $(video.videoTextContainer).show();
              }
              ...
              // Hide video until tapped to play on mobile
              $(video.element).closest('.plyr__video-wrapper').hide();
              ...
              player.on('play', function() {
                // Show video
                $(video.element).closest('.plyr__video-wrapper').show();
          ...
          if (video.aspectRatio) {
            player.ratio = video.aspectRatio;
          }

          /*
          Show Video Controls
          Video controls get hidden using a css class
          ...
          */
          ...
          if (video.playButton) {
            $(video.playButton).on('click', function() {
  ...
  setup: function($video, id) {
    ...
    var videoProvider = $videoWrapper.data('video-provider');
    var aspectRatio = $videoWrapper.data('aspect-ratio');
    var isHTML = $videoWrapper.data('video-html5');
    ...
    var videoObject = {
      ...
      videoTextContainer: $videoTextContainer,
    ...
    this.videoElementObjects.push(videoObject);
  }
}
  ...
  init: function(){
    ...
    if (galleriesArray.length > 0) {
      ...
      $.each(galleriesArray, function(index, galleryObject) {
        ...
        // If product media is supported for the gallery, then also launch product media
        if (galleryObject.productMediaSupported) {
          product.launchProductMedia(galleryObject);
        }

        // Hover to Zoom now controlled by section settings
  ...
  loadProductRecommendations : function() {
        ...
        request.onload = function() {
          if (request.status >= 200 && request.status < 300) {
            ...
            // Since recommended products get loaded in after, we must run Plyr.setup on these new videos
            var videosInRecommendedProducts = document.querySelectorAll('.recommended-products-section [data-html5-video] video, .recommended-products-section [data-youtube-video]')
...
var utils = {
  ...
  launchVideoPlayer: function(Plyr) {
    var videos = document.querySelectorAll('[data-html5-video] video, [data-youtube-video]')

    videoPlayers = Plyr.setup(videos, {
      controls: controls,
      fullscreen: {
        enabled: true,
        fallback: true,
...
$(function() {
  ...
  window.lazySizesConfig.minSize = 300;
  ...
  if ($('.shopify-section.page-details-section, .product-gallery').length){
    // Don't apply mediaWrapper
  } else if ($('body').hasClass('index')) {
    $('.featured-products-section iframe[src*=youtube]:not(.plyr--video iframe[src*=youtube]), .featured-products-section iframe[src*=vimeo]').mediaWrapper({
      intrinsic: true
    });
  } else {
    $('iframe[src*=youtube]:not(.plyr--video iframe[src*=youtube]), iframe[src*=vimeo]').mediaWrapper({
      intrinsic: true
    });
  }
...
function refreshCart(cart) {
  ...
  if ($cartBtn) {
    ...
    if (cart.item_count == 0) {
      $cart.append('<li class="mm-label empty_cart"><a href="'+ Shopify.routes.cart_url +'">Your Cart is Empty</a></li>');
      $cartCount.remove();
    } else {
      ...
      if (cart.cart_level_discount_applications.length > 0) {
        cart_items_html += '<li class="mm-label">' + '<a href="'+ Shopify.routes.cart_url +'">' + '<strong>Discount</strong>' + '</a>' + cartDiscountMessage + '</li>'
      }

      cart_items_html += '<li class="mm-label">' + '<p class="mm-counter"><span class="money">' + Shopify.formatMoney(cart.total_price, $('body').data('money-format')) + '</span></p>' + '<a href="'+ Shopify.routes.cart_url +'">' + '<strong>Subtotal</strong>' + '</a>' + '</li>' + '<li class="mm-subtitle clearfix">';
      ...
      cart_items_html += '<li class="mm-subtitle mm-subtitle--actions">' + '<input type="submit" class="action_button" value="Checkout" />' + '<a href="'+ Shopify.routes.cart_url +'" class="action_button action_button--edit">Edit Cart</a>' + '</li>';
...
selectCallback = function(variant, selector) {
  ...
  if (variant && variant.available == true) {
    ...
    if(variant.price < variant.compare_at_price){
      ...
      $('.was_price', $product).html('<span class="money" data-currency-'+ dataCurrency +'="' + Shopify.formatMoney(variant.compare_at_price, $('body').data('money-format')) + '">' + Shopify.formatMoney(variant.compare_at_price, $('body').data('money-format')) + '</span>');
    ...
    if (variant.price > 0) {
      var dataCurrency = $('body').data('active-currency');
      $('.current_price', $product).html('<span class="money" data-currency-'+ dataCurrency +'="' + Shopify.formatMoney(variant.price, $('body').data('money-format')) + '">' + Shopify.formatMoney(variant.price, $('body').data('money-format')) + '</span>');
    } else {
